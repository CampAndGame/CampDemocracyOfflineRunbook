...
- **`docker-compose.yml`** — 6 контейнеров (postgres + redis + democracy-api[standalone] + democracy-player + democracy-admin + nginx-proxy), версия сервиса `${VERSION:-1.0.35}`.
...
## Быстрый старт
1. `./build-dist.sh` (на машине с интернетом + доступом к реестру) → тянет все образы → `democracy-standalone-v1.0.35.zip`.
...
- `VERSION` — democracy-service (по умолчанию `1.0.35`);
